...
# SOURCE THESE FILES BEFORE RUNNING MAKE
# source /tools/xilinx/Vitis_HLS/2023.2/settings64.sh
# source /tools/xilinx/xrt/setup.sh
...
KERNEL_NAME=kernel_$(KERNEL)
KERNEL_SRC_FILES=./$(KERNEL).cpp
...
CXX_ACC_SIM_FLAGS=$(CXX_ACC_FLAGS) -D MCC_SIM -D MCC_ACC_H_FILE="__merlin$(KERNEL_NAME)_sim.h"                   -l$(KERNEL_NAME)_sim
CXX_ACC_HW_FLAGS=$(CXX_ACC_FLAGS) -D MCC_ACC_H_FILE="__merlin$(KERNEL_NAME).h"                   -l$(KERNEL_NAME)
...
$(EST_OUTPUT) : $(ACCGEN_OUTPUT).mco ########################
	$(EXEC) $^ --report=estimate $(LNK_OPT) -p=$(PATH_D) --kernel_frequency 250  
...
$(ACCGEN_OUTPUT).mco : $(KERNEL_SRC_FILES) ########################
	$(EXEC) -c $^ -D $(VENDOR) -o $(ACCGEN_OUTPUT) $(CMP_OPT) -p=$(PATH_D) $(KERNEL_INC_DIR) 

$(SIMGEN_OUTPUT).mco : $(KERNEL_SRC_FILES)
	$(EXEC) -D MCC_SIM -c $^ -D $(VENDOR) -o $(SIMGEN_OUTPUT) $(CMP_OPT) -p=$(PATH_D) $(KERNEL_INC_DIR) 
...
$(AFIGEN_OUTPUT).awsxclbin : $(BITGEN_OUTPUT)
	$(SDK_DIR)/../SDAccel/tools/create_sdaccel_afi.sh -xclbin=$(BITGEN_OUTPUT) 	             -o=$(AFIGEN_OUTPUT) -s3_bucket=fcs-fpga -s3_dcp_key=dcp -s3_logs_key=log
